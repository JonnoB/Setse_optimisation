...
```{r}
packages <- c("tidyverse", "igraph", "igraphdata", "minpack.lm", "ggraph", "profvis", "microbenchmark", "ggraph")
...
g3 <- g2 %>% Proportional_Load(., 3, PowerFlow = "power_flow", Link.Limit = "edge_capacity")
...
#  filter(node == 109) %>%
ggplot(., aes(y = elevation, x = t, colour = as.numeric(node), group = node)) + geom_line() +
...
compare_expanded  %>%
  ...
  ggplot(aes(x = (Iter), y = log10(MAPE), colour = factor(ec))) + geom_line()+
...
test_loop_base<- function(){
  ...
  template_nodes_df <-template_nodes_df <-node_bicomp_relation %>%
    ungroup() %>%
    group_by(component) %>%
    mutate(path_component = (name %in% component_specific_floor_ceiling_nodes_2)*1,
           path_component2 = sum(path_component),
           target_node_name = names(node_path[length(node_path)]))
  ...
  return(list(floor_df = floor_df_2, ceiling_df = ceiling_df_2))
}


test_loop_1<- function(){
  for(i in 1:length(bicomp_shortest_paths$vpath) ){
  target_shortest_path <-i
...
test_loop_base_df <- test_loop_base()
test_loop_1_df <- test_loop_1()

time1 <-system.time(test_loop_base_df <- test_loop_base())
time2 <-system.time(test_loop_1_df <- test_loop_1())

time1/time2

all.equal(test_loop_1_df$floor_df, test_loop_base_df$floor_df)
all.equal(test_loop_1_df$ceiling_df, test_loop_base_df$ceiling_df)
