...
g2 <- readRDS(file.path("/home/jonno/Dropbox/IEEE_Networks/power_grid_graphs",  "IEEE_118_igraph.rds" ))#"IEEE_118_igraph.rds"))
...
It can be difficult to know how to set the convergence criteria for the sysem aka how to set tstep and coef_drag. It may be possible to automate this using the nelder-mead derivitive free optimisation method
```{r}

nelder_mead_tstep_drag <- function(par, tol = 2e-3){
  #This is a simplified version that only takes two arguments the vector of parameters and the system tolerance
  
  #get the embeddings for the network
  #negative time and drag are forbidden
  emdedded_network <- SETS_embedding(g, 
                                     force =force,
                                     flow = flow,
                                     distance = distance,
                                     edge_name = edge_name,
                                     capacity = capacity,
                                     k = "k",
                                     tstep = ifelse(par[1]<0,0, par[1] ), 
                                     tol = tol,
                                     max_iter = 5e4 ,
                                     coef_drag = ifelse(par[2]<0,0, par[2] ),
                                     mass = mass,
                                     sample = 100
  )
  
  #find the total static force in the network. this is the value to be minimised
  total_static_force <- sum(abs(emdedded_network$node_embeddings$static_force))
  
  #this puts a hard minimum of the convergence tolerance as an output. this is becuase all that matters is that the static
  #force is smaller than the tolerance not that it is minimised
  Out <- ifelse(total_static_force < tol, tol, total_static_force)
  
  return(Out)
}


test <- optim(par = c(0.01, 4), 
              nelder_mead_tstep_drag, 
              method = "L-BFGS-B", 
              upper = c(0.1, 5),
              lower = c(0,0),
              control = list(trace = 3, maxit = 20)
)
...
emdedded_network <- SETS_embedding(g, 
                                   ...
                                   tol = tol,
                                   max_iter = 1e5 ,
                                   coef_drag = test$par[2],
                                   mass = mass,
                                   sample = 100
)

  
  test2 <- emdedded_network$node_embeddings
  log10(sum(abs(test2$static_force)))
  
  
df <- expand.grid(x = 0:100, y = 0:100)
df$z <- runif(nrow(df))
