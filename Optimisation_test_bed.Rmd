...
origin_block_base_node_name <-get.vertex.attribute(g, 
                                                   "name")[-which(get.vertex.attribute(List_of_BiConComps[[OriginBlock_number]],"name") %in% articulation_nodes_df$name)] %>% 
  min
...
for(i in 1:length(unique(node_bicomp_relation$name)) ){
...
g2 <- set.vertex.attribute(g, name = "color", value = NA)  %>% 
  set.vertex.attribute(., name = "color", index =  component_specific_floor_ceiling_nodes, value = "path artic") %>%
...
ggraph(g2, layout = layout_with_fr(g)) +
  ...
  geom_node_point( aes(color = floor ),
                   size = 2.5) +
