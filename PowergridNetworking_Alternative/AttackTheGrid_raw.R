...
#' 
#This does no speed ups and updates all subgraphs all the time

AttackTheGrid_raw <- function(NetworkList,
                          AttackStrategy,
                          referenceGrid = NULL,
                          MinMaxComp = 0.0,
  ...
  if(!GridCollapsed){
    ...
    if(CascadeMode){
      ...
      gCasc <- Cascade_raw(NetworkList  = gCasc,
                       Iteration = 0,
                       StopCascade = Inf,
                       g0 = g,
                       Demand = Demand,
  ...
  if( !(FractGC < MinMaxComp | length(NetworkList2)-1==TotalAttackRounds| GridCollapsed| TopoStability) ){
    NetworkList2 <- AttackTheGrid_raw(NetworkList = NetworkList2,
                                  AttackStrategy,
