...
Cascade_loop <- function(NetworkList,
                    Iteration = 0,
                    StopCascade = Inf,
                    g0 = NULL,
                    AZero,
    ...
    gList <- 1:nrow(SlackRefCasc) %>%
      map(~{
        ...
        if(SlackRef$Nodes > 1){
          ...
          gsubset <- PowerFlow2(gsubset, SlackRef$name, AZero = AZero, EdgeName, VertexName, Net_generation, power_flow)
        }
...
#If there is a reference graph that has subcomponents the subcomponents that have been changed by targeting are re-combined into the
  ...
  edgesequal <-all(!Components_differ_fix(g2, g, EdgeName = EdgeName))# length(DeleteEdges$index)==0
