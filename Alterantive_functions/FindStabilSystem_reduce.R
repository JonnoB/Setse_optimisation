...
FindStabilSystem <- function(node_status, ten_mat, non_empty_matrix, kvect, dvect, mass,
 ...
 # Iter <- 1
  ...
  temp <- Reduce(function(state, Iter){ 
    ...
    NodeList2 <- state$NodeList
    #####
    #create the tension matrix
    #####
    #dz is the change in eleveation
    dzvect <- state$NodeList[non_empty_matrix[,2],2] - state$NodeList[non_empty_matrix[,1],2] #The difference in height between adjacent nodes 
    ...
    NodeList2[,2] <- state$NodeList[,4]*tstep +0.5*state$NodeList[,8]*tstep*tstep + state$NodeList[,2] #Distance/elevation s1 = ut+0.5at^2+s0
    NodeList2[,4] <- state$NodeList[,4] + state$NodeList[,8]*tstep #velocity v1 = v0 +at
    NodeList2[,6] <- state$NodeList[,1] + state$NodeList[,3] #static force 
    ...
    NodeList2[,9] <- state$NodeList[,9] + tstep #current time #This may not be neccessary but doesn't really hurt
    
    
    state$network_dynamics[Iter,]<-  c(Iter, #Iteration
    ...
    return(list(system_stable = system_stable, NodeList =  NodeList2, network_dynamics =  state$network_dynamics))
    }, 
    1:max_iter, list(system_stable = FALSE, NodeList = NodeList, network_dynamics = network_dynamics)
    )

  
  #Early termination causes NA values. These are removed by the below code
  #
  network_dynamics <- as_tibble(temp$network_dynamics) %>%
    filter(complete.cases(.))
  
  Out <- list(network_dynamics = as_tibble(network_dynamics), 
              node_status = bind_cols(node_status[,"node",drop=FALSE] , 
                                      as_tibble(temp$NodeList)
              )
  )
